...
//main declarations---------------------
  ...
  const float SCREEN_SCALE_FACTOR = 1.0f;
  ...
  const Uint16 INFORMATION_BAR_FONT_SIZE = 1;//Uint16(SCREEN_SCALE_FACTOR * (float(28) / 5.0f));
...
void mainSetup(void){
  ...
  initRandomNumbers();
  Serial.println("start mainSetup()");
  ...
  if (!init(&initIn)){
    ...
    else
    {
      ...
      if(1==0){
        //drwaw info bar
        testdrawtext("SCORE 0000 LIVES ", TFT_GREEN);

        uint16_t x = 100;
        uint16_t y = 0;

        widowHandler->drawXBitmap(x, 0, imageHandlers[CRAFT].image.imgData, imageHandlers[CRAFT].w, imageHandlers[CRAFT].h, TFT_GREEN);

        x = x + imageHandlers[CRAFT].w + 2;

        widowHandler->drawXBitmap(x, 0, imageHandlers[CRAFT].image.imgData, imageHandlers[CRAFT].w, imageHandlers[CRAFT].h, TFT_GREEN);

        //draw craft
        x = (SCREEN_WIDTH - imageHandlers[CRAFT].w) / 2;
        y = SCREEN_HEIGHT - imageHandlers[CRAFT].h;
        widowHandler->drawXBitmap(x, y, imageHandlers[CRAFT].image.imgData, imageHandlers[CRAFT].w, imageHandlers[CRAFT].h, TFT_GREEN);

        x = (SCREEN_WIDTH - imageHandlers[CRAFT].w) / 2;
        y = SCREEN_HEIGHT - imageHandlers[CRAFT].h;

        const uint8_t X0 = (SCREEN_WIDTH - (8 * imageHandlers[ALIEN].w + 7 * 2)) / 2;
        const uint8_t Y0 = 2 * imageHandlers[ALIEN].h;   
        for (uint8_t i = 0; i < 5; i ++){
          y = Y0 + i * (imageHandlers[ALIEN].h + 2);
          for (uint8_t j = 0; j < 8; j ++){
            x = X0 + j * (imageHandlers[ALIEN].w + 2);
            
            widowHandler->drawXBitmap(x, y, imageHandlers[ALIEN].image.imgData, imageHandlers[ALIEN].w, imageHandlers[ALIEN].h, TFT_GREEN);
      ...
      while ((false == gameOver) && (false == quit)){
        ...
        if (true == gameOver) {
          //Show game over message
          {
            ...
            showMessageIn.size = 2;//Uint16(SCREEN_SCALE_FACTOR * (float(60) / 5.0f));
        ...
        //delay(15);
        sleep_delay(30); //Si hayb problemas de fluidez camnbiar por sleep_delay(15);
...
void testdrawtext(char *text, uint16_t color) {
  widowHandler->setCursor(0, 0);
  widowHandler->setTextColor(color);
  widowHandler->setTextWrap(false);
  widowHandler->print(text);
}
...
bool loadMedia(LoadMediaIn_t* loadMediaIn) {
	...
	if (loadMediaIn->imageHandlers[ALIEN].image.imgData == NULL) {
	...
	if (loadMediaIn->imageHandlers[CRAFT].image.imgData == NULL) {
		printf("Unable to load image %s!\n", "img/craft.bmp");
	}
	...
	if (loadMediaIn->imageHandlers[ALIEN_SHOOT].image.imgData == NULL) {
		printf("Unable to load image %s!\n", "img/alien_shoot.bmp");
	}
  ...
  colorKey.r = 0xFF;
	...
	if (loadMediaIn->imageHandlers[CRAFT_SHOOT].image.imgData == NULL) {
		printf("Unable to load image %s!\n", "img/craft_shoot.bmp");
	}
...
void close(CloseIn_t* p_closeIn) {
	...
	for (Uint16 i = 0; i < IMAGES_NUMBER; i++) {
		SDL_DestroyTexture(p_closeIn->imageHandlers[i].image->imgData);
		p_closeIn->imageHandlers[i].image.imgData = NULL;
	}

	//Deallocate informationBarr text images and fonts
	for (Uint16 i = 0; i < INFROMATION_BAR_TEXTS_MESSAGES_NUMBER; i++) {
		SDL_DestroyTexture(p_closeIn->p_gameScreen->iformationBar.textMessage[i].image->text);
		p_closeIn->p_gameScreen->iformationBar.textMessage[i].image.text = NULL;
	}
...
void gameScreenInit(GameScreen_t* p_gameScreen, GameScreenInitIn_t* p_gameScreenInitIn){
	...
	p_gameScreen->iformationBar.textMessage[SCORE_LABEL] = 
  getTextImage("SCORE", p_gameScreen->iformationBar.p_font, fontColor, p_gameScreenInitIn->screenRenderer);
	...
	p_gameScreen->gameArea.y = 2 * p_gameScreen->iformationBar.area.h;
	p_gameScreen->gameArea.w = p_gameScreenInitIn->total.w;
	p_gameScreen->gameArea.h = (p_gameScreenInitIn->total.h - p_gameScreen->gameArea.y);
}
...
void informationBarUpdate(InformationBar_t* p_informationBar, Craft_t* p_craft, SDL_Renderer* screenRenderer) {
	...
	rectangle.x = p_informationBar->area.x + p_informationBar->textMessage[SCORE_LABEL].w + Uint16(float(1) * p_craft->image->screenScale) + Uint16(float(3) * p_craft->image->screenScale);
	...
	{
		char score_s[]= "0000"; 
    intToArray(p_craft->score, score_s);
		p_informationBar->textMessage[SCORE_VALUE] = getTextImage(score_s, p_informationBar->p_font, fontColor, screenRenderer);
	...
	rectangle.y = p_informationBar->area.y + Uint16(float(2) * p_craft->image->screenScale);
...
void keyboardRead(Craft_t* p_craft, Shoots_t* p_craftShoots) {
	const Uint32 KEYBOARD_SCAND_PERIOD_MS = 20;
  const bool ALLOW_AUTO_SHOOT = false;
	Uint32 currentTime = 0;
	static Uint32 lastKeyboardRead = 0;
  static Uint8 a_key_previous_state = 0;
  static Uint8 b_key_previous_state = 0;
  bool craftShootConditions = false;
    ...
    if (true == ALLOW_AUTO_SHOOT){
      craftShootConditions = (
        (1 == keyboardState[SDL_SCANCODE_A]) ||
        (1 == keyboardState[SDL_SCANCODE_B])
        );
    }
    else{
        craftShootConditions = (
          ((1 == keyboardState[SDL_SCANCODE_A]) && (0 == a_key_previous_state)) ||
          ((1 == keyboardState[SDL_SCANCODE_B]) && (0 == b_key_previous_state))
          );

        a_key_previous_state = keyboardState[SDL_SCANCODE_A];
        b_key_previous_state = keyboardState[SDL_SCANCODE_B];
    }

		if (true == craftShootConditions) {
...
void craftInit(Craft_t* p_craft, CraftInitIn_t* p_craftInitIn){
	...
	p_craft->w = craftW;
	p_craft->h = craftH;
	p_craft->x = p_craftInitIn->p_gameScreen->gameArea.x + ((p_craftInitIn->p_gameScreen->gameArea.w - craftW) / 2);
...
void craftShoot(Shoots_t* p_shoots, Craft_t* p_craft) {
	const Uint32 SHOOT_DELAY_MS = 250;
...
Sint16 getShootColumn(Aliens_t* p_aliens) {
	...
	if (activeColumnsNumber > 0) {
		columnNumberSelected = getRandomNumber(1, activeColumnsNumber);//Uint16(rand() % activeColumnsNumber) + 1;
...
void aliensShoot(Shoots_t* p_shoots, Aliens_t* p_aliens){
	...
	if (currentTime > (p_shoots->lastShootTime + p_shoots->automaticShootPeriod)) {
		...
		shootValue = getRandomNumber(1, 100);//Uint16(rand() % 100) + 1;
...
void writeTryAgainMessage(void){
    //Show Press a yey and message
  {
    ShowMessageIn_t showMessageIn;
    showMessageIn.y = 25;
    showMessageIn.position = HORIZONTAL_CENTERED;
    showMessageIn.text = "Press a key and";
    showMessageIn.size = 1;//Uint16(SCREEN_SCALE_FACTOR * (float(60) / 5.0f));
    showMessageIn.fontColor = { 0x00,0xFF,0x00,0xFF};
    showMessage(&showMessageIn, screenRenderer);
    SDL_RenderPresent(screenRenderer);
  }
...
enum WaitForTryAgainStates{
  INIT = 0,
  WAIT_FOR_TRY_AGAIN_MESSAGE,
  WAIT_FOR_PRESS_KEY
};

void waitForTryAgain(void){
  
  const Uint32 WAIT_FOR_TIME_MS = 1500;
  static Uint8 state = INIT;
  Uint32 currentTime = 0;
  static Uint32 initialTime = 0;
  const Uint8* keySatatus;
  ...
  switch (state){
    case INIT:
      ...
      state = WAIT_FOR_TRY_AGAIN_MESSAGE;
      break;

    case WAIT_FOR_TRY_AGAIN_MESSAGE:
      //..
      
      if(currentTime > WAIT_FOR_TIME_MS + initialTime){
        writeTryAgainMessage();
        state = WAIT_FOR_PRESS_KEY;
      }
      break;
    case WAIT_FOR_PRESS_KEY:
      keySatatus =  SDL_GetKeyboardState(NULL);
